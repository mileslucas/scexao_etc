...
from scexao_etc.models import TYPES, PICKLES_MAP, load_pickles, VEGASPEC, color_correction
...
def calculate_results(
    ...
    for inst_filt, inst_elem in inst.filters.items():
        ...
        inst_mag = obs_mag - zp
        inst_counts = 10 ** (-0.4 * inst_mag) * inst.factor
        ...
        fig = px.imshow(np.log10(noisy_psf_adu), origin="lower", aspect="equal", color_continuous_scale="cividis")
        fig.update_layout(
            coloraxis_colorbar=dict(
                title=dict(text="log10(flux)", side="right"),
            ),
        )
        tbls.append(tbl)
        figs.append(fig)
...
def Fiq(lam, seeing, airmass, L0=46):
    ...
    print(fatm)
    ftel = 1.028 * np.rad2deg(lam / 7.92e9) * 3600
    print(ftel)
...
with right_col:
    ...
    exptime = st.number_input(
        "Exposure Time", value=0.1, min_value=7.2e-6, max_value=1800.0
    )
st.header("Results")
